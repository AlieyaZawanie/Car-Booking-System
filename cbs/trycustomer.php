...
// Retrieve the booking status for the current customer

$uid = $_SESSION['uid'];
$uid = $_SESSION['uid'];
  $sql = "SELECT b_status FROM tb_booking WHERE b_customer= '$uid'";
  $result = mysqli_query($con, $sql);
  $b_status = mysqli_fetch_array($result)['b_status'];

  if ($b_status == '2') {
    echo "<script>alert('Your booking has been approved by the admin.');</script>";
  }

  $sql1="SELECT * FROM tb_user WHERE u_id='$uid'";
$result1=mysqli_query($con,$sql1);
$row1=mysqli_fetch_array($result1); ?>
 <BR> <h3 class="" style="align-items: center;" >WELCOME!<?php echo $row1['u_name'];?> </h3><BR>
        ...
        <section class="py-4 py-xl-5">
            <section class="py-4 py-xl-5">
                <div class="container">
                    <div class="row">
                        <div class="col-md-5">
                            <lottie-player src="https://assets9.lottiefiles.com/packages/lf20_PBgNop.json" mode="normal"></lottie-player>
                        </div>
                        <div class="col-md-6 col-xl-4">
                            <div>
                                <form class="p-3 p-xl-4" method="post">
                                    ...
                                    <div class="mb-3"><label class="form-label" for="email">Email</label><input id="email" class="form-control" type="email" name="email" /></div><label class="form-label" for="message">Message</label><input id="email-4" class="form-control" type="email" name="email" />
                                    <div class="mb-3"></div><label class="form-label" for="message">Message</label><input id="email-3" class="form-control" type="email" name="email" /><button class="btn btn-primary" type="submit">Send </button>
                                </form>
                            </div>
                        </div>
                    </div>
